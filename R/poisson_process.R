...
#'@export
poisson_process <- function(rate, shape = 1, level = .95){
  pois_crit <- data.frame("Lower" = qpois((1-level)/2, lambda = 1/rate),
                          "Upper" = qpois(1 - (1-level)/2, lambda = 1/rate))

  exp_crit <- data.frame("Lower" = critical_values_exp(level, rate)[1],
                         "Upper" = critical_values_exp(level, rate)[2])

  gamma_crit <- data.frame("Lower" = critical_values_gamma(level, rate, shape)[1],
                           "Upper" = critical_values_gamma(level, rate, shape)[2])

  pois_plot <- plot_poisson(rate, level)
  exp_plot <- plot_exponential(rate, level)
  gamma_plot <- plot_gamma(rate, shape, level)
  results <- list("Inputs" = data.frame("Rate" = rate,
                                        "Shape" = shape,
                                        "Level" = level),
                  "Critical Values" = list("Poisson" = pois_crit,
                                           "Exponential" = exp_crit,
                                           "Gamma" = gamma_crit),
                  "Poisson" = pois_plot,
                  "Exponential" = exp_plot,
                  "Gamma" = gamma_plot)
  return(results)
}
...
#' Display a Poisson plot
#'
#' @param rate rate of occurrence
#' @param level confidence level
#'
#' @return a poisson plot
plot_poisson <- function(rate, level){
    x <- 0:(1/rate * 7)
...
#' Display Exponential plot
#'
#' @param rate rate of occurrence
#' @param level confidence level
#'
#' @return an exponential plot
plot_exponential <- function(rate, level){
    x = 0:(rate*40)
    y <- dexp(x, rate = rate)
...
#'
#' @return a gamma plot
plot_gamma <- function(rate, shape, level){
    x = 0:(shape*10)
